...
class SaleOrder(models.Model):
    ...
    shopify_fulfillment_sync_response=fields.Char(string="Shopify Fulfillment Status",default="Not Yet Fulfilled")
    # @api.onchange('partner_id')
    def hit_order_fulfillment(self):
        for rec in self:
            if rec.state not in ['sale','done']:
                continue
            shopify_instance=rec.shopify_instance_id
            order_mapping=self.env['eg.sale.order'].search([('odoo_order_id','=',rec.id)])
            if not shopify_instance or not order_mapping:
                continue
            shop_url=shopify_instance.get_shopify_url(shopify_instance)
            order_id=order_mapping.inst_order_id
            headers = {
            'X-Shopify-Access-Token': shopify_instance.shopify_password,
            'Content-Type': 'application/json',
                }
            fulfillment_orders = requests.get(shop_url + '/orders/'+ order_id +'/fulfillment_orders.json',headers=headers).json()     
            # raise UserError(str(fulfillment_orders))
            try:
                fulfillment_id = fulfillment_orders['fulfillment_orders'][0]['id']
            except:
                rec.shopify_fulfillment_sync_response="Error: No Fulfillment Order Found!"
                continue
            status = fulfillment_orders['fulfillment_orders'][0]['status']
            if status=="closed":###if already closed, no need to create fulfillment again.
                rec.shopify_fulfillment_sync_response="Already Closed"
                continue
            
            rec.shopify_fulfillment_sync_response=str(status)
            location_id = shopify_instance.shopify_location_id.location_id  
            ...
            payload = {
                "fulfillment": 
                    {            
                        ...
                        "tracking_info":
                            {                
                            "url": str(shopify_instance.company_id.website),
                            "company": str(shopify_instance.company_id.name),
                            "number": 0,            
                            },
            ...
            try:
                ...
                rec.shopify_fulfillment_sync_response=status
            except:
                rec.shopify_fulfillment_sync_response="Error:"+str(r.status_code)+" "+str(r.text)
        ...
        for instance_id in instance_ids:
            ...
            if shop_connection:
                ...
                while count > 0:
                    try:
                        if next_page_url:
                            response = Order.find(status='any',from_=next_page_url)
                        elif cron == "yes" and instance_id.spf_last_order_date:
                            response = Order.find(limit=250,status='any',
                                                          created_at_min=instance_id.spf_last_order_date.strftime(
                                                              "%Y-%m-%dT%H:%M:%S"))
                        else:
                            response = shopify.draft_order.DraftOrder.find(status='any',limit=250)
                        ...
                        for order in response:
                            ...
                            if order.get("customer") and order.get("customer").get("default_address"):
                                ...
                                if not eg_order_id:  # TODO New Change
                                    instance_name="Shopify : " + instance_id.name
                                    order_id = self.search([("name", "=", order.get("name")),('company_id','=',instance_id.company_id.id),('source_name','=',instance_name)])
                                    eg_journal_id = None
                                    ...
                                    if order_id:
                                        ...
                                        if partner_id:
                                            ...
                                            create_date = datetime.strptime(create_date,
                                                                            "%Y-%m-%d %H:%M:%S")
                                            for partner in billing_partner_id:
                                                billing_partner=partner
                                                break
                                            for partner in shipping_partner_id:
                                            ...
                                            order_dict = {"partner_id": partner_id.id,
                                                          ...
                                                          'owner_id':instance_id.owner_id.id,
                                            ...
                                            if order.get('shipping_lines'):
                                                shipping_product = self.env['product.product'].search([('name','=',order.get('shipping_lines')[0]['title'])])
                                                if not shipping_product:
                                                    shipping_product = self.env['product.product'].create({
                                                        "name":order.get('shipping_lines')[0]['title'],
                                                        'company_id':instance_id.company_id.id,
                                                          'source_name':"Shopify : "+instance_id.name,
                                                          'shopify_instance_id':instance_id.id,
                                                ...
                                                order_line_id = self.env["sale.order.line"].create(
                                                            ...
                                                            "product_uom_qty": 1,
                                                            "price_unit": order.get('shipping_lines')[0]['price'],
